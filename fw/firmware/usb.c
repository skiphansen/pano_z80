...
#ifdef   USB_DEBUG
//#define   USB_PRINTF(fmt, args...)   term_print(fmt , ##args)
         ...
         void *data, int len, int *actual_length, int timeout)
{
   ...
   while(ticks_ms() - StartTime < timeout) {
      ...
      USB_PRINTF("%s: submit_bulk_msg returned %d \n",__FUNCTION__,Err);
              ...
              unsigned char *buffer, int cfgno)
{
   ...
   if (result < 9) {
      if (result < 0)
         printf("unable to get descriptor, error %d\n",
            dev->status);
      else
...
int usb_new_device(struct usb_device *dev)
{
   ...
   USB_PRINTF("new device vid = %x pid = %x usb = %x class = %x\n",
...
#ifdef   USB_HUB_DEBUG
//#define   USB_HUB_PRINTF(fmt, args...)  term_print(fmt , ##args)
         ...
         unsigned short *portstat)
{
   ...
   for (tries = 0; tries < MAX_TRIES; tries++) {
      ...
      if (usb_get_port_status(dev, port + 1, &portsts) < 0) {
         USB_HUB_PRINTF("get_port_status failed status %d\n",
               dev->status);
         return -1;
      }
